...
namespace MysteriousAlchemy.Projectiles
{
    public class FrostEssenceController : ModProjectile
    {
        ...
        private void DrawVortex(Vector2 Center, Vector2 Toward, float scale, float VortexRate)
        {
            ...
            MainUtil.DrawEntityInWorld(Main.spriteBatch, CircleTex, Center - Main.screenPosition, Color.White, CommenPolarVortex, 0, scale, -ToMouse.ToRotation(), MathHelper.PiOver4 * ((ToMouse.SafeNormalize(Vector2.One).X) < 0 ? -1 : 1), () =>
            {
        ...
        private void SpownVortexDust(Vector2 Center, Vector2 Toward, float Radium)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Vector2 FinalPosition = MainUtil.MartixTrans(DustSpownPosition, -ToMouse.ToRotation(), MathHelper.PiOver4 * ((ToMouse.SafeNormalize(Vector2.One).X) < 0 ? -1 : 1));
        ...
        private void DrawLaser(Vector2[] LaserCenter)
        {
            ...
            MainUtil.DrawProjectileTrail(LaserCenter, Mainshape, MainMask, MainColor, 35, -(float)Main.time / 600f);
        }
