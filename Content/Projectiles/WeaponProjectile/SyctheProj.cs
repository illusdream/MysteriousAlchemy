...
{
    public class SyctheProj : ModProjectile
    {
        ...
        float DefaultWeaponSize = 2;
        float StaticWeaponSize = 2;
        ...
        public override void SetDefaults()
        {
            ...
            SlashMainColor = AssetUtils.GetColorBar("DeathSickle");
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            switch (attackType)
            {
                case AttackType.Swing:
                    DamageAttackTypeModify = 1;
                    break;
                case AttackType.UpToDown:
                    DamageAttackTypeModify = 2;
                    break;
                case AttackType.Spin:
                    DamageAttackTypeModify = 4;
                    break;
                default:
                    break;
            }
            ...
            {
                float NPCLifeLoseModify = Math.Clamp((1 - target.GetLifePercent()) * 3, 1, 10);
        ...
        public virtual void SpownDust()
        {
            if (Main.rand.NextFloat() * 2f < Projectile.Opacity)
            {
                ...
                Color dustColor = Color.Lerp(Color.Purple, Color.Black, Main.rand.NextFloat() * 0.3f);
                ...
                coloredDust.fadeIn = 0.7f + Main.rand.NextFloat() * 0.15f;
                coloredDust.noGravity = true;
                if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity)
                {
                    Dust.NewDustPerfect(HandPos + WeaponTopPos * 6 / 7f * Main.rand.NextFloat(0.5f, 1), 27, CurretAngle.ToRotationVector2().RotatedBy(MathHelper.PiOver2 * Clockwise) * Main.rand.NextFloat(1, 4), 100, Color.Purple * Projectile.Opacity, 1.2f * Projectile.Opacity).fadeIn = 0.7f;
                }
            }
        }
        //是否可以造成伤害
