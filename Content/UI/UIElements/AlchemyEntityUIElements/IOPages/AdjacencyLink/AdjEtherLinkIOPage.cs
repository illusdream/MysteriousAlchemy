...
using MysteriousAlchemy.Content.UI.UIElements.AlchemyEntityUIElements.AdjLinkShelfs;
...
namespace MysteriousAlchemy.Content.UI.UIElements.AlchemyEntityUIElements.IOPages.AdjacencyLink
{
    public class AdjEtherLinkIOPage : LinkIOPage
    {
        ButtomWithImageText AddNewLink;
        public AdjEtherLinkIOPage(Pull_outButtom pull_OutButtom, int AnimationTime, Vector2 StaticEdgeRange, Vector2 ActiveEdgeRange, Vector2 PulloutVector, Vector2 bottomOriginOffest) : base(pull_OutButtom, AnimationTime, StaticEdgeRange, ActiveEdgeRange, PulloutVector, bottomOriginOffest)
        {
        ...
        {
            UIList.Clear();
            if (AlchemySystem.etherGraph.FindAdjacencyNodes(unicode, out var result))
            {
                result.ForEach(StartUnicode =>
                {
                    if (AlchemySystem.etherGraph.FindLink(StartUnicode, unicode, out var link))
                    {
                        LinkShowshelf instancce = new AdjEtherLinkShowShelf(StartUnicode, unicode);
